...
<div className="mt-2">
     ...
     <div>
        ...
        <input className="font-xs font-semibold text-gray-800 outline-none  focus:outline-none" type="text" placeholder="" value={project}  onChange={(e)=>setProject(e.target.value)}/>
        </div>
     </div>
  {/* Time tags */}
     ...
     <div className="border border-solid border-gray-200 rounded-md">
          ...
          <input type="datetime-local" value={(startdatetime || '').toString().substring(0, 16)}  onChange={handleStart} />
     </div>
     <div className="border border-solid border-gray-200 rounded-md">
          <p className="text-base font-semibold">End Time</p>
          <input type="datetime-local" value={(enddatetime || '').toString().substring(0, 16)}  onChange={handleEnd} />
     </div>
     </div>
         
    <div className="mt-4  px-2 py-1"> 
    <textarea value={notes} onChange={(e)=>setNotes(e.target.value)}   rows="3" class="px-0 py-1 w-full text-sm text-gray-900  bg-white border  focus:outline focus:outline-blue-300 rounded-lg  border-gray-200" placeholder="Notes.."></textarea>
    </div>
     {/* Status */}
    ...
    <p className="font-bold text-base text-black">Status</p>
         <select className=" font-semibold text-sm text-black hover:cursor-pointer" onChange={(e)=>setStatus(e.target.value)}>
    ...
    {/* Delete option */}
    <div className="px-2 py-1 w-fit flex gap-x-2 items-center mt-4" onClick={()=>handleDelete}>
