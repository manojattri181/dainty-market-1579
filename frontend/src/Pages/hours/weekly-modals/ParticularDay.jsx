...
const ParticularDay = ({ week, day, date }) => {
  ...
  for (var i = 0; i < t.length; i++) {
    ans.push(
      new Date(`${t[i].endDate}`).getTime() -
        new Date(`${t[i].startDate}`).getTime()
    );
  }
  var sum = 0;
  ...
  return (
    <div
      className="flex  flex-col  justify-center content-center w-1/6 border border-inherit"
      style={{ background: one == two ? "#fbfbf4" : "" }}
    >
