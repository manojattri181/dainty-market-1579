...
const TerminalNavbar = () => {
  ...
  return (
    <div className="min-w-full h-10 px-4 py-3 flex justify-between items-center bg-white-100">
      <div className="flex items-center justify-start">
        <img className="w-36" src={logo} alt="" />
        <div className="w-fit flex pl-8 text-center items-center justify-start gap-x-2">
          <Link to="/hours">
            <p className="w-fit px-2 py-2 font-bold text-sm  text-gray-700 rounded-md hover:bg-gray-200   hover:cursor-pointer">
              Hours
            </p>
          </Link>
          <Link to="/projects">
          ...
          <div className="relative">
            <button
              className="w-fit px-2 py-2 font-bold text-sm  text-gray-700 rounded-md hover:bg-gray-200    hover:cursor-pointer"
              onMouseOver={() => setReports(true)}
              onMouseLeave={() => setReports(false)}
            >
              Reports
            </button>
            {reports && (
              <div className="absolute z-30">
                <div
                  className="w-48  pb-1 z-40 -ml-14 rounded-xl bg-white justify-center items-center shadow shadow-gray-300 "
                  onMouseOver={() => setReports(true)}
                  onMouseLeave={() => setReports(false)}
                >
      ...
      <div className="flex justify-between items-center gap-x-4">
        ...
        <button className="hidden rounded-full bg-gray-300">
          <FaPauseCircle size="20px" />
        </button>
        <button className="w-fit px-3 py-1  text-white rounded-md font-semibold text-sm bg-gradient-to-r from-red-600 to-red-400">
          GET PRO
        </button>
        <button className="rounded-full  bg-white">
          <BsFillArrowDownSquareFill color="#00cccc" size="25px" />
        </button>
        <button className="rounded-full  border-solid border border-black">
          <FiUser size="25px" />
        </button>
      </div>
    </div>
